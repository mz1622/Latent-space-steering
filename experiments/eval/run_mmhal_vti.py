...
def eval_model(args):
    ...
    # Determine device (use MPS on Mac if available, otherwise CPU)
    ...
    input_images, input_ids = get_demos(args, image_processor, model, tokenizer, device=device)
    ...
    if args.alpha_image != 0:
        vti_vision, _ = obtain_visual_vti(
            model, input_images, rank=1, device=device
            )

        visual_direction = vti_vision[1:]

    if args.alpha_image != 0:
        add_vti_layers(model.model.vision_tower.vision_tower.vision_model, torch.stack([visual_direction],dim=1).to(device), alpha = [args.alpha_image])

    if args.alpha_text != 0:

        vti_text, _ = obtain_textual_vti(
            model, input_ids, input_images, rank=1, device=device
            )
        textual_direction = vti_text[1:]

    if args.alpha_text != 0:
        add_vti_layers(model, torch.stack([textual_direction],dim=1).to(device), alpha = [args.alpha_text])
