    ...
    def forward(self, x):
        if self.vti_direction is not None:
            ...
            if isinstance(self.lam, (list, tuple)):
                lam_list = list(self.lam)
            else:
                lam_list = [self.lam]


            for i in range(len(self.vti_direction)):
                lam_i = lam_list[0] if len(lam_list) == 1 else lam_list[i]
                if x.size(1) < 2:
                    ...
                    y += lam_i * lambda_sim * F.normalize(self.vti_direction[i], dim=-1).repeat(1,x.shape[1],1)
                else:
                    lambda_sim = 1.0
                    y += lam_i * lambda_sim * F.normalize(self.vti_direction[i], dim=-1)
...
def add_vti_layers(model: PreTrainedModel, vti_drections: Tensor, alpha: list):
    layers = get_layers(model)
    mlp_keywords = ["mlp", "feedforward", "ffn"]
    if len(vti_drections) != len(layers):
        if len(vti_drections) > len(layers):
            extra = len(vti_drections) - len(layers)
            print(
                f"Warning: vti_drections has {len(vti_drections)} layers but model has "
                f"{len(layers)}; trimming first {extra} direction(s) to align."
            )
            vti_drections = vti_drections[extra:]
        else:
            raise ValueError(
                f"vti_drections has {len(vti_drections)} layers but model has {len(layers)}; "
                "cannot pad missing directions. Recompute directions for this model."
            )
