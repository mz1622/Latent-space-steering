...
def get_prompts(args, model, tokenizer, data_demos, question, model_is_llaval=True, device="cuda"):
    if model_is_llaval:
        ...
        if hasattr(model.config, 'mm_use_im_start_end'):
            ...
            input_ids_positive = [tokenizer_image_token(p, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(device) for p in prompts_positive]
            input_ids_negative = [tokenizer_image_token(p, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(device) for p in prompts_negative]
...
def get_demos(args, image_processor, model, tokenizer, patch_size = 14, file_path = './experiments/data/hallucination_vti_demos.jsonl', model_is_llaval=True, device="cuda"): 
    ...
    input_ids = get_prompts(args, model, tokenizer, data_demos, question, model_is_llaval=model_is_llaval, device=device)

    return inputs_images, input_ids


def get_hiddenstates(model, inputs, image_tensor, device="cuda"):
        h_all = []
        with torch.no_grad():
            for example_id in tqdm(range(len(inputs)), desc="Computing textual hidden states", ncols=100):
                ...
                for style_id in range(len(inputs[example_id])):
                    if image_tensor is None:
                        ...
                        img_input = image_tensor[example_id][-1].unsqueeze(0)
                        if device != "cpu":
...
def obtain_textual_vti(model, inputs, image_tensor, rank=1, device="cuda"):
    hidden_states = get_hiddenstates(model, inputs, image_tensor, device=device)
    print("Computing textual VTI direction via PCA...")
    hidden_states_all = []
    ...
    print("✓ Textual VTI direction computed")
...
def get_visual_hiddenstates(model, image_tensor, model_is_llaval=True, device="cuda"):
    h_all = []
    with torch.no_grad():
        if model_is_llaval:
            try:
        ...
        for example_id in tqdm(range(len(image_tensor)), desc="Computing visual hidden states", ncols=100):
            embeddings_for_all_styles= []
            for style_id in range(len(image_tensor[example_id])):
                if isinstance(image_tensor[example_id][style_id], list):
                    ...
                    trials_iter = tqdm(image_tensor[example_id][style_id], desc=f"  Processing trials (demo {example_id+1})", leave=False, ncols=100)
                    for image_tensor_ in trials_iter:
                        img_input = image_tensor_.unsqueeze(0).to(device)
                        if model_is_llaval and device != "cpu":
                            img_input = img_input.half()
                        if model_is_llaval:
                            h_ = vision_model(
                                img_input,
                            ...
                            _, h_ = vision_model(img_input)
                        h.append(h_)
                    h = average_tuples(h)
                else:
                    img_input = image_tensor[example_id][style_id].unsqueeze(0).to(device)
                    if model_is_llaval and device != "cpu":
                        img_input = img_input.half()
                    if model_is_llaval:
                        h = vision_model(
...
def obtain_visual_vti(model, image_tensor, rank=1, model_is_llaval=True, device="cuda"):

    hidden_states = get_visual_hiddenstates(model, image_tensor, model_is_llaval = model_is_llaval, device=device)
    print("Computing visual VTI direction via PCA...")
    ...
    print("✓ Visual VTI direction computed")
    return direction, reading_direction
